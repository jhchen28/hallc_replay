...
void SIDISkaoncoinTime(Int_t runNumber=1889){
    ...
    SHMSpartMass = 493.677/1000.0; // kaon mass in GeV/c^2 
    ...
    TCut aeroCut;
	 ...
	 cerCut = "P.hgcer.npeSum > 0.1 && H.cer.npeSum > 0.0";
         aeroCut= "P.aero.npesum < 0.1";
    ...
    tree->Draw("P.hod.starttime >> SHMShodoStartTime",  eBetaCut && pBetaCut && cerCut && calCut && hodoTimeCut && aeroCut);
    TCanvas *canvas2 = new TCanvas("canvas2","canvas2");
    tree->Draw("H.hod.starttime >> HMShodoStartTime",  eBetaCut && pBetaCut && cerCut && calCut && hodoTimeCut && aeroCut);
	...
	tree->GetEntry(event);
	    ...
	    if( !(PgtrBetaMes > 0.5 && PgtrBetaMes < 1.5                       // proton beta cut
		  && HgtrBetaMes > 0.5 &&  HgtrBetaMes < 1.5                   // e beta cut
		    && HcerNpeSum > 0. && PhgcerNpeSum > 0.1   // Cerenkov counter cut for e- and pion/kaon
		  && PaeroNpeSum < 0.1                // aerogel cut for kaons
