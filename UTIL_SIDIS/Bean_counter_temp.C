...
void Bean_counter_temp(Int_t runNumber, Int_t shms_polarity=1,Int_t last_event=-1){
...
//event_cut = epievent_cut || epevent_cut || ekevent_cut ;
    ...
    if (shms_polarity == 1) epievent_cut = paeronpe > 3. && hms_elec && hpdelta_cut ;
    if (shms_polarity == 2) epievent_cut = pcernpe ==0 && hms_elec && hpdelta_cut ;
    //    epievent_cut = paeronpe > 0. && pcernpe > 0. && hcaletot > 0.6 && pcaletot > 0.6 && hpdelta_cut ;
    epevent_cut  = paeronpe <= 3.  && hms_elec  && hpdelta_cut ;
    ekevent_cut  = paeronpe <= 3.0 && hms_elec   && hpdelta_cut ;
    ...
    if (abs(coinP-3) <1.5) h_aerosum_ecaltot_p->Fill(pcaletot,paeronpe);
    if (abs(coinP-3) <1.5) h_aerosum_hgcer_p->Fill(pcernpe,paeronpe);
    if (abs(coinK-3) <1.5) h_aerosum_ecaltot_k->Fill(pcaletot,paeronpe);
    if (abs(coinPi-3) <1.5) h_aerosum_ecaltot_pi->Fill(pcaletot,paeronpe);
...
TCanvas *can4 = new TCanvas ("can4",Form("can4 Run: %d",runNumber),700,1000);
  //*******************************************************************************
  ...
  Double_t max_cnt=h1_epi_PcointimeROC2_noPID->GetMaximum();
  TLine *cline[8];
  for (Int_t ip=0;ip<9;ip++) {
    ...
    if ((ip-4)==0) cline[ip]->SetLineColor(6);
  }
...
//
  ...
  TCanvas *can2 = new TCanvas ("can2",Form("can2 Run: %d",runNumber),700,1000);
can2->Divide(1,2);
  //*******************************************************************************
...
if (shms_polarity == 1)  {
  ...
  TLine *pline = new TLine(good_coin_peak_time+tdiff,0.,good_coin_peak_time+tdiff,max_cnt/2.);
  pline->Draw();
  pline->SetLineColor(3);
  ...
  TLine *kline = new TLine(good_coin_peak_time+tkdiff,0.,good_coin_peak_time+tkdiff,max_cnt/2.);
  kline->Draw();
